...
int main(){
    ...
    output1=fopen("D:\\ScoreRanking.txt","w");
    if(output1==NULL){
    ...
    for(int i=1;i<=30;i++){
        fprintf(output1,"%s %d %.2f %.2f %.2f %.2f\n",Student[i].name,Student[i].code,Student[i].math,Student[i].eng,Student[i].C,Student[i].all_score);
    }
    fclose(output1);
